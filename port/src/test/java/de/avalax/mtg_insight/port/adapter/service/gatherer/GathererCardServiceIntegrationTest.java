...
import static de.avalax.mtg_insight.port.adapter.service.TestHelper.assertAbilities;
...
import static de.avalax.mtg_insight.port.adapter.service.TestHelper.assertNoneAbility;
...
public class GathererCardServiceIntegrationTest {
    ...
    public void cardFromCardname_shouldReturnCreatureWithSingleMana() throws Exception {
        ...
        assertAbilities(card, Ability.FIRST_STRIKE, Ability.HEXPROOF);
    ...
    @Test
    public void cardFromCardname_shouldReturnCreatureWithoutAbilities() throws Exception {
        String cardname = "Runeclaw Bear";

        Card card = cardService.cardFromCardname(cardname);

        assertCard(card, cardname, Creature.class);
        assertMana(card, Mana.COLORLESS, Mana.GREEN);
        assertCardColor(card, Color.GREEN);
        assertNoneAbility(card);
        assertCreature(card, 2, 2);
    }
}